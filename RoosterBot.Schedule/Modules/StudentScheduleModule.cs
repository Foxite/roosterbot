	...
	public class StudentScheduleModule : ScheduleModuleBase {
		...
		public async Task ShowFutureCommand([Range(1, 52)] int amount, string unit, StudentSetInfo info) {
			if (unit == "uur") {
				...
				if (result.Success) {
					ScheduleRecord record = result.Value;
					if (record != null) {
						await RespondRecord(string.Format(ResourcesService.GetString(Culture, "ScheduleModuleBase_InXHours"), record.StudentSetsString, amount), info, record);
					} else {
		...
		private async Task RespondDay(StudentSetInfo info, DateTime date) {
			...
			if (result.Success) {
				...
				string response;
				if (records.Length == 0) {
					response = string.Format(ResourcesService.GetString(Culture, "StudentScheduleModule_RespondDay_NoRecordAtRelattive"), ScheduleUtil.GetRelativeDateReference(Culture, date));
