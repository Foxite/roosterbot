		...
		public async Task<CommandResult> AfterCommand() {
			...
			if (query == null) {
				...
				if (query.Identifier is TeacherInfo) {
					query.Identifier = Context.ServiceProvider.GetService<TeacherNameService>().GetRecordFromAbbr(Context.ChannelConfig.ChannelReference, query.Identifier.ScheduleCode);
				}
